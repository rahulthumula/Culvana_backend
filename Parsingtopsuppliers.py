    ...
    try:
        ...
        for page_num in sorted(pages_content.keys()):
            ...
            try:
                ...
                logging.info(f"Page {page_num} - Text lines: {len(page_data['text'])}")
                
                # Format page content
                page_text = format_page_content(page_data, page_num)
                logging.info(f"Formatted page {page_num} content length: {len(page_text)}")
                
                # Process in chunks if needed
                if len(page_text) > 16000:  # GPT-4o token limit safety
                    logging.warning(f"Page {page_num} content too large, splitting...")
                    # Handle large content
                    current_invoice = handle_large_page(page_text, current_invoice, all_invoices)
                else:
                    # Send to GPT
                    page_result = send_to_gpt(page_text)
                    logging.info(f"Got result for page {page_num}")
...
def main(invoice_file_path):
    try:
        ...
        csv_filename = f"{invoice_name}{timestamp}.csv"
...
if __name__ == "__main__":
    try:
        ###"C:\Users\rahul\Downloads\RESTAURANT DEPOT INVOICE 2.pdf"
        ###"C:\Users\rahul\Downloads\RESTAURANT DEPOT INVOICE 1.pdf"
